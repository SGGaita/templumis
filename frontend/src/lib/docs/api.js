export const apiTitle = "API documentation";
export const apiSubtitle =
  "REST API for TemplumIS 0.2.0. All application JSON is served under `/api`.";

export const apiBlocks = [
  { type: "h2", id: "conventions", text: "Conventions" },
  {
    type: "ul",
    items: [
      "**Base path:** `/api` (via Nginx or the Next.js rewrite to the FastAPI service).",
      "**Interactive OpenAPI:** `/docs` and `/openapi.json` on the API host (Swagger UI).",
  ...
  { type: "h2", id: "auth", text: "Authentication" },
  {
    type: "p",
    text: "Login returns a JWT (`sub` = user id, `role`, `exp`). Store it as `templumis_token` (the web client) and send it on subsequent calls. Tokens last **24 hours**.",
  },
  {
    type: "code",
...
Response:
{
  ...
  { type: "h2", id: "endpoints", text: "Endpoints by module" },

  { type: "h3", id: "ep-global", text: "Global admin — `/api/global-admin`" },
  {
    type: "table",
  ...
  { type: "h3", id: "ep-inst", text: "Institution admin — `/api/institution`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
      ...
      ["POST", "/users", "Create user (email, full_name, password, role, optional category and student number)."],
  ...
  { type: "h3", id: "ep-students", text: "Students CRUD — `/api/students`" },
  {
    type: "p",
    text: "Staff category required. Results are scoped to the caller's institution. Query params on list: `skip`, `limit`, `search`, `status`, `compliance_status`, `program_id`, `cohort_id`.",
  },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
  ...
  { type: "h3", id: "ep-sis", text: "SIS / LMS — `/api/sis-lms`" },
  {
    type: "p",
    text: "Operational student, course, scholarship-application, and analytics APIs used by the portals. Scholarship draft/submit lives here alongside Excel-backed SIS reads.",
  },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
  ...
  { type: "h3", id: "ep-programs", text: "Scholarship programmes — `/api/sis-lms/scholarships`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h3", id: "ep-triage", text: "Triage — `/api/sis-lms/financial-aid/triage`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
      ["GET", "/settings", "Triage / blind-review config."],
      ["PATCH", "/settings", "Update config (reviewer count, salt, cycle year, weights)."],
  ...
  { type: "h3", id: "ep-eval", text: "Evaluation — `/api/sis-lms/financial-aid/evaluation`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h3", id: "ep-awards", text: "Awards — `/api/sis-lms/financial-aid/awards`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  {
    type: "p",
    text: "Financial-aid dashboard: **GET** `/api/sis-lms/financial-aid/dashboard`.",
  },

  { type: "h3", id: "ep-grants", text: "Grants — `/api/sis-lms/grants`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h3", id: "ep-journey", text: "Student journey — `/api/student-journey`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h3", id: "ep-support", text: "Student support — `/api/student-support`" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h3", id: "ep-rankings", text: "Rankings" },
  {
    type: "table",
    headers: ["Method", "Path", "Description"],
    rows: [
  ...
  { type: "h2", id: "calling", text: "Calling the API from the web client" },
  {
    type: "p",
    text: "The Next.js app calls relative `/api/…` via `apiFetch` in `frontend/src/lib/api.js`. In local dev, Next rewrites `/api/*` to the FastAPI process. In Docker, Nginx proxies `/api/` to the backend container. Multipart uploads set `Authorization` but omit JSON `Content-Type` so the browser can attach the boundary.",
  },
  {
    type: "code",
    label: "Example",
    text: `curl -X POST http://localhost/api/auth/login \\

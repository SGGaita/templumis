const en = {
  // ─── Language picker ───────────────────────────────────────────────────────
  language: {
    selectLabel: "SELECT LANGUAGE",
    comingSoon: "Translation feature coming soon. Currently showing English content.",
  },

  // ─── Common / shared ───────────────────────────────────────────────────────
  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    close: "Close",
    confirm: "Confirm",
    back: "Back",
    next: "Next",
    submit: "Submit",
    create: "Create",
    update: "Update",
    search: "Search",
    filter: "Filter",
    export: "Export",
    import: "Import",
    loading: "Loading...",
    loadingPortal: "Loading portal...",
    error: "Something went wrong. Please try again.",
    noData: "No data available",
    viewAll: "View All",
    actions: "Actions",
    status: "Status",
    name: "Name",
    email: "Email",
    role: "Role",
    date: "Date",
    amount: "Amount",
    type: "Type",
    description: "Description",
    department: "Department",
    profile: "Profile",
    logout: "Logout",
    notifications: "Notifications",
    account: "Account",
    copyright: "TemplumIS. All rights reserved.",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service",
    contact: "Contact",
    documentation: "Documentation",
    footer: {
      usefulLinks: "Useful links",
      home: "Home",
      faqs: "FAQs",
      bookDemo: "Book a demo",
      addressOrg: "Training Centre in Communication",
      addressBuilding: "Gecaga Institute Building",
      addressCity: "Nairobi, Kenya",
    },
    required: "Required",
    optional: "Optional",
    yes: "Yes",
    no: "No",
    active: "Active",
    inactive: "Inactive",
    pending: "Pending",
    approved: "Approved",
    rejected: "Rejected",
    draft: "Draft",
    completed: "Completed",
    enrolled: "Enrolled",
    open: "Open",
    closed: "Closed",
    new: "New",
    all: "All",
  },

  // ─── Home / landing ────────────────────────────────────────────────────────
  home: {
    nav: {
      loginBtn: "Login",
      signupBtn: "Sign up",
      openMenu: "Open menu",
      closeMenu: "Close menu",
    },
    hero: {
      tagline: "Powering smarter higher education",
      subtitle: "Institutional intelligence for enrollment, student success, and research",
      description:
        "Transforming siloed institutional data into a unified intelligence layer for enrollment, grants, scholarships, and student success.",
      getStartedBtn: "Get started",
      bookDemoBtn: "Book a demo",
    },
    modules: {
      heading: "Core Modules",
      subheading: "Five integrated modules covering the complete institutional data lifecycle.",
      enrollment: {
        title: "Enrollment & Student Success",
        description:
          "Monitor and optimize the student journey from admission to graduation with TTD analytics and early-warning dashboards.",
      },
      financial: {
        title: "Scholarship & Financial Aid",
        description:
          'Manage the full scholarship lifecycle — fund creation, applications, awarding, and compliance — via the "Finance Bridge."',
      },
      support: {
        title: "Student Support",
        description:
          "Empower students with milestone tracking, self-service tools, nudge notifications, and academic support ticketing.",
      },
      grants: {
        title: "Grants & Research",
        description:
          "Track research investment, grant burn rates, publication output mapping, and ethics/IRB compliance alerts.",
      },
      rankings: {
        title: "University Rankings",
        description:
          "Track institutional performance across major global ranking systems with indicator readiness, targets, and live dashboards.",
      },
    },
  },

  // ─── Auth ──────────────────────────────────────────────────────────────────
  auth: {
    login: {
      title: "Welcome",
      subtitle: "Sign in to your account to continue",
      emailLabel: "Email Address",
      passwordLabel: "Password",
      signInBtn: "Sign In",
      signingIn: "Signing in...",
      noAccount: "Don't have an account?",
      signUpLink: "Sign up",
      adminAccess: "Administrator Access",
      globalAdmin: "Global Admin",
      institutionAdmin: "Institution Admin",
      invalidCredentials: "Invalid credentials",
      accountNotConfigured:
        "Your account type is not configured for this login. Try Institution Admin or Global Admin login.",
      forgotPassword: "Forgot password?",
    },
    forgotPassword: {
      title: "Forgot password",
      subtitle: "Enter your email and we will send a reset link if an account exists.",
      submitBtn: "Send reset link",
      sendingBtn: "Sending...",
      sent: "If an account exists for that email, we sent a reset link. Check your inbox.",
      emailRequired: "Enter your email address",
    },
    resetPassword: {
      title: "Set a new password",
      subtitle: "Choose a new password for your account.",
      newPassword: "New password",
      confirmPassword: "Confirm password",
      submitBtn: "Update password",
      savingBtn: "Updating...",
      passwordHelper: "At least 8 characters",
      passwordTooShort: "Password must be at least 8 characters",
      passwordsNoMatch: "Passwords do not match",
      invalidLink: "This reset link is invalid or has expired. Request a new one.",
      success: "Your password was updated. You can sign in now.",
      signInBtn: "Sign in",
      missingParams: "This reset link is incomplete. Request a new one from the login page.",
    },
    globalAdminLogin: {
      subtitle: "Global admin login",
      signInBtn: "Sign In",
    },
    institutionLogin: {
      subtitle: "Institution admin login",
      signInBtn: "Sign In",
    },
    signup: {
      title: "Create Your Account",
      subtitle: "Join your institution on TemplumIS",
      steps: ["Account Type", "Personal Information", "Account Details"],
      step0: {
        heading: "Select Your Account Type",
        studentTitle: "Student Account",
        studentDesc: "Access your academic records, scholarships, and support services",
        staffTitle: "Admin Staff Account",
        staffDesc: "Manage institutional operations, students, and resources",
      },
      step1: {
        heading: "Personal Information",
        studentIdLabel: "Student Registration Number",
        studentIdHelper: "Enter your official student ID/registration number",
        emailLabel: "Email Address",
        emailHelper: "Use your institutional email address",
        emailAutoFilled: "Auto-filled from institutional records",
        emailAutoFillPending: "Will be auto-filled after student ID verification",
        fullNameLabel: "Full Name",
        fullNameAutoFilled: "Auto-filled from institutional records",
        fullNameAutoFillPending: "Will be auto-filled after student ID verification",
        roleLabel: "Department/Role",
        roles: {
          registrar: "Registrar's Office",
          scholarship_office: "Financial Aid Officer",
          student_services: "Student Services",
          research_office: "Research Office",
          vice_chancellor: "Executive Office",
        },
      },
      step2: {
        heading: "Account Security",
        passwordLabel: "Password",
        passwordHelper: "At least 6 characters",
        confirmPasswordLabel: "Confirm Password",
        confirmPasswordHelper: "Re-enter your password",
        passwordsMatch: "Passwords match",
        passwordsNoMatch: "Passwords do not match",
        summaryTitle: "Account Summary",
        summaryType: "Account Type:",
        summaryName: "Name:",
        summaryEmail: "Email:",
        summaryRegNumber: "Registration Number:",
        summaryDepartment: "Department:",
        studentType: "Student",
        staffType: "Admin Staff",
        createBtn: "Create Account",
        creatingBtn: "Creating account...",
      },
      alreadyHaveAccount: "Already have an account?",
      signInLink: "Sign in",
      adminAccess: "Administrator Access",
      globalAdmin: "Global Admin",
      institutionAdmin: "Institution Admin",
      errors: {
        selectAccountType: "Please select an account type",
        enterStudentId: "Please enter your student registration number",
        invalidStudentId: "Please enter a valid student registration number",
        studentInfoNotVerified:
          "Student information could not be verified. Please ensure your student ID is correct.",
        fillRequiredFields: "Please fill in all required fields",
        invalidInstitutionalEmail: "Please enter a valid institutional email",
        useValidEmail: "Please use a valid institutional email",
        passwordsNoMatch: "Passwords do not match",
        passwordTooShort: "Password must be at least 6 characters",
        studentIdRequired: "Student registration number is required",
        failedToCreate: "Failed to create account",
        failedToValidateEmail: "Failed to validate email",
        failedToValidateId: "Failed to validate student ID",
      },
    },
    verifyEmail: {
      title: "Verify Your Email",
      subtitle: "We sent a 6-digit code to your email address",
      codeLabel: "Verification Code",
      verifyBtn: "Verify",
      verifyingBtn: "Verifying...",
      resendCode: "Resend Code",
      backToSignup: "Back to Sign Up",
    },
  },

  // ─── Student portal ────────────────────────────────────────────────────────
  student: {
    portal: "Student Portal",
    loading: "Loading student portal...",
    nav: {
      groups: {
        academic: "ACADEMIC",
        financial: "FINANCIAL",
        opportunities: "OPPORTUNITIES",
        support: "SUPPORT & RESOURCES",
        research: "RESEARCH",
        funding: "FUNDING",
      },
      items: {
        dashboard: "Dashboard",
        courses: "My Courses",
        grades: "Grades & Results",
        attendance: "Attendance",
        financials: "Financials",
        scholarships: "My Scholarships",
        available: "Browse Scholarships",
        grants: "My Grants",
        funding: "Browse Grants",
        support: "Support Hub",
        library: "Library Resources",
        writing: "Writing Centre",
        supervisor: "My Supervisor",
        pgsupport: "PG Support Office",
      },
    },
    topbar: {
      notifications: "Notifications",
      account: "Account",
      myProfile: "My Profile",
    },
    dashboard: {
      welcomeBack: "Welcome back,",
      cumulativeGpa: "Cumulative GPA",
      outOf: "out of 4.00",
      scholarshipSubmitted: "Your scholarship application was submitted. Track status below in My Applications.",
      researchProgramme: "Research programme",
      next: "Next:",
      viewGrantInfo: "View grant information",
      stats: {
        creditsEnrolled: "Credits Enrolled",
        completed: "completed",
        attendanceRate: "Attendance Rate",
        sessions: "sessions",
        balanceDue: "Balance Due",
        paid: "paid",
        scholarships: "Scholarships",
        publications: "Publications",
        conferences: "conferences",
        appliedToFees: "applied to fees",
      },
      courses: {
        title: "Current Courses",
        enrolledSemester: "enrolled this semester",
        noActive: "No active enrolments",
        headers: { course: "Course", code: "Code", credits: "Credits", grade: "Grade" },
      },
      attendance: {
        title: "Attendance",
        overall: "Overall",
        byCourse: "Attendance by Course",
      },
      grades: {
        title: "Grade Distribution",
      },
      financial: {
        title: "Financial Summary",
        totalFees: "Total Fees",
        paid: "Paid",
        scholarship: "Scholarship",
        balance: "Balance Due",
      },
      scholarshipStatus: {
        title: "Scholarship Status",
        active: "Active Scholarship",
        noScholarship: "No active scholarship",
        browseCta: "Browse available scholarships",
      },
      academicProfile: {
        title: "Academic Profile",
        programme: "Programme",
        department: "Department",
        year: "Year",
        gpa: "GPA",
        credits: "Credits",
        nationality: "Nationality",
        enrolled: "Enrolled",
      },
    },
    profile: {
      title: "My Profile",
      academicInfo: "Academic Information",
      researchProfile: "Research Profile",
      financialSummary: "Financial Summary",
      fields: {
        programme: "Programme",
        department: "Department",
        yearOfStudy: "Year of Study",
        gpa: "GPA",
        credits: "Credits",
        nationality: "Nationality",
        enrollmentDate: "Enrollment Date",
        studentId: "Student ID",
        dissertation: "Dissertation Title",
        supervisor: "Supervisor",
        researchArea: "Research Area",
        proposalStatus: "Proposal Status",
        totalFees: "Total Fees",
        paid: "Paid",
        scholarship: "Scholarship Applied",
        balanceDue: "Balance Due",
      },
    },
    courses: {
      title: "My Courses",
      statCards: {
        totalEnrolled: "Total Enrolled",
        completed: "Completed",
        credits: "Total Credits",
      },
      tabs: { all: "All", enrolled: "Enrolled", completed: "Completed" },
      table: {
        course: "Course",
        code: "Code",
        department: "Department",
        credits: "Credits",
        attendance: "Attendance",
        grade: "Grade",
        status: "Status",
      },
      noResults: "No courses found",
    },
    grades: {
      title: "Grades & Results",
      stats: {
        cumulativeGpa: "Cumulative GPA",
        coursesGraded: "Courses Graded",
        highestGrade: "Highest Grade",
        avgScore: "Avg Score",
        creditsGraded: "Credits Graded",
      },
      charts: {
        distribution: "Grade Distribution",
        scoreTrend: "Score Trend",
      },
      table: {
        course: "Course",
        code: "Code",
        score: "Score",
        grade: "Grade",
        points: "Points",
        progress: "Progress",
      },
      noGrades: "No grades recorded yet",
    },
    attendance: {
      title: "Attendance",
      overall: "Overall Attendance",
      compliant: "Compliant",
      notCompliant: "Below Requirement",
      required: "Required: 75%",
      byCourse: "Attendance by Course",
      table: {
        course: "Course",
        present: "Present",
        total: "Total",
        absent: "Absent",
        percentage: "Percentage",
      },
      noData: "No attendance records",
    },
    financials: {
      title: "Financials",
      balanceWarning: "You have an outstanding balance. Please contact the finance office.",
      stats: {
        totalFees: "Total Fees",
        paid: "Paid",
        scholarship: "Scholarship",
        balanceDue: "Balance Due",
      },
      feeBreakdown: "Fee Breakdown",
      paymentProgress: "Payment Progress",
      feeRecords: "Fee Records",
      paymentHistory: "Payment History",
      paymentMethods: {
        all: "All",
        bank: "Bank",
        mpesa: "M-Pesa",
        cash: "Cash",
      },
      table: {
        date: "Date",
        method: "Method",
        reference: "Reference",
        amount: "Amount",
      },
      noPayments: "No payment records",
    },
    scholarships: {
      title: "My Scholarships",
      browseTitle: "Browse Scholarships",
      forYou: "For You",
      allScholarships: "All Scholarships",
      tabs: {
        drafts: "Drafts",
        inReview: "In Review",
        offers: "Offers",
        credited: "Credited",
      },
      apply: "Apply",
      viewDetails: "View Details",
      deleteDialog: {
        title: "Delete Draft",
        message: "Are you sure you want to delete this draft application?",
      },
      offerDialog: {
        title: "Scholarship Offer",
        accept: "Accept Offer",
        decline: "Decline",
      },
      empty: "No applications found.",
      browseCta: "Browse Available Scholarships",
      pipelineGuide: "Application Pipeline",
    },
    grants: {
      title: "My Grants",
      browseTitle: "Browse Grant Opportunities",
      apply: "Apply",
      viewDetails: "View Details",
      lifecycle: "View Lifecycle",
      empty: "No grant applications found.",
      browseCta: "Browse Opportunities",
      stages: {
        proposal: "Proposal",
        budget: "Budget",
        compliance: "Compliance",
        review: "Review",
        award: "Award",
      },
    },
    support: {
      title: "Support Hub",
      journey: "My Journey",
      resources: "Support Resources",
      alerts: "Alerts & Notifications",
      resourceCards: {
        scholarships: { title: "Scholarships", desc: "Browse and apply for financial aid opportunities" },
        library: { title: "Library", desc: "Access digital resources and research databases" },
        advisor: { title: "Academic Advisor", desc: "Schedule a meeting with your academic advisor" },
        financialAid: { title: "Financial Aid", desc: "Review your financial aid status and options" },
        studentServices: { title: "Student Services", desc: "General student support and welfare" },
        grants: { title: "Research Grants", desc: "Funding opportunities for postgraduate research" },
        writingCentre: { title: "Writing Centre", desc: "Academic writing support and resources" },
        supervisor: { title: "My Supervisor", desc: "Contact and meet with your research supervisor" },
        pgSupport: { title: "PG Support Office", desc: "Postgraduate-specific support services" },
      },
    },
    library: {
      title: "Library Resources",
      searchPlaceholder: "Search resources...",
      openResource: "Open",
      table: {
        title: "Title",
        type: "Type",
        subject: "Subject",
        accessNotes: "Access Notes",
      },
      noResults: "No resources found",
    },
  },

  // ─── Staff portal ──────────────────────────────────────────────────────────
  staff: {
    portal: "Staff Portal",
    financialAidPortal: "Financial Aid",
    loading: "Loading portal...",
    nav: {
      groups: {
        studentSuccess: "STUDENT SUCCESS",
        studentCohorts: "STUDENT COHORTS",
        enrollmentRecords: "ENROLLMENT & RECORDS",
        financialAid: "FINANCIAL AID",
        grants: "GRANTS & RESEARCH",
        analytics: "ANALYTICS",
      },
      items: {
        dashboard: "Dashboard",
        atRisk: "At-Risk Students",
        support: "Student Support",
        scholarships: "Scholarships",
        undergraduates: "Undergraduates",
        postgraduates: "Postgraduates",
        enrollment: "Enrollment",
        import: "Import Data",
        financialAid: "Financial Aid",
        applications: "Applications",
        triage: "Triage",
        committee: "Committee",
        decisions: "Decisions",
        configure: "Configure",
        opportunities: "Opportunities",
        grantApplications: "Grant Applications",
        grantConfigure: "Configure Grants",
        grantLifecycle: "Grant Lifecycle",
        rankings: "University Rankings",
        analytics: "Analytics",
        executiveBriefing: "Executive briefing",
        nsfasVisualAnalytics: "NSFAS Visual Analytics",
      },
    },
    topbar: {
      notifications: "Notifications",
      account: "Account",
      profile: "Profile",
      logout: "Logout",
      dashboard: "Dashboard",
    },
    dashboard: {
      title: "Staff Dashboard",
      quickAccess: "Quick Access",
      stats: {
        totalStudents: "Total Students",
        activeCourses: "Active Courses",
        totalEnrollments: "Total Enrollments",
        programs: "Programs",
      },
      charts: {
        enrollmentTrend: "Enrollment Trend",
        genderDistribution: "Gender Distribution",
        programDistribution: "Program Distribution",
        topNationalities: "Top Nationalities",
        complianceStatus: "Compliance Status",
      },
    },
    vcDashboard: {
      badge: "Vice Chancellor",
      title: "Institutional decision dashboard",
      subtitle: "Exception signals, portfolio health, and levers for leadership — not operational queues.",
      dataAsOf: "Data as of {when}",
      openBriefing: "Open full executive briefing",
      loadError: "Failed to load leadership metrics",
      status: {
        onTarget: "On target",
        watch: "Watch",
        actionNeeded: "Action needed",
      },
      attention: {
        title: "Needs attention",
      },
      pulse: {
        title: "Institution pulse",
        totalStudents: "Total students",
        activeStudents: "{count} active",
        onTrack: "On track",
        atRisk: "At-risk students",
        atRiskShare: "{pct}% of enrolment",
        feeCollection: "Fee collection",
        avgGpa: "Institution GPA",
        rankingReadiness: "Ranking readiness",
        vsTarget: "Target {target}",
      },
      success: {
        title: "Student success",
        subtitle: "Compliance mix, risk drivers, and retention",
        cta: "Review at-risk",
        onTrack: "On track",
        atRisk: "At risk",
        critical: "Critical",
        finances: "Finances",
        attendance: "Attendance",
        academic: "Academic",
        probation: "{count} on probation or suspended",
        retention: "1-yr retention {pct}%",
        graduation: "4-yr graduation {pct}%",
      },
      portfolio: {
        title: "Portfolio & access",
        subtitle: "Programme concentration and aid pipeline",
        ugPg: "Undergraduate / postgraduate",
        international: "International students",
        female: "Female share",
        aidPending: "{count} pending",
        aidCommittee: "{count} ready for committee",
        aidAwarded: "{count} awarded",
        aidCta: "Aid decisions",
      },
      trend: {
        title: "Enrollment by intake year",
        subtitle: "Live cohort sizes from institutional SIS records",
        students: "Students",
      },
      rankings: {
        title: "University rankings & readiness",
        subtitle: "Institution readiness across ranking systems",
        cta: "Open University Rankings",
        overall: "Overall readiness",
        unavailable: "Rankings readiness data is not available. Open University Rankings after uploading institutional rankings data.",
        grantsNote: "Research grants: {pending} pending · {approved} approved",
      },
    },
    enrollment: {
      title: "Student Enrollment",
      search: "Search students...",
      importCsv: "Import CSV",
      exportCsv: "Export CSV",
      syncData: "Sync Data",
      table: {
        name: "Name",
        studentId: "Student ID",
        program: "Program",
        year: "Year",
        status: "Status",
        actions: "Actions",
      },
      detail: {
        title: "Student Detail",
        profile: "Profile",
        courses: "Courses",
        grades: "Grades",
        attendance: "Attendance",
        financials: "Financials",
      },
      import: {
        title: "Import Students",
        uploadLabel: "Upload CSV File",
        validate: "Validate",
        preview: "Preview",
        importBtn: "Import",
        mapping: "Column Mapping",
      },
    },
    atRisk: {
      title: "At-Risk Students",
      subtitle: "Students flagged for intervention",
      riskFactors: {
        financial: "Financial Risk",
        attendance: "Attendance Risk",
        academic: "Academic Risk",
      },
      intervene: "Intervene",
      referToSupport: "Refer to Support",
      table: {
        student: "Student",
        riskScore: "Risk Score",
        factors: "Risk Factors",
        lastActivity: "Last Activity",
        actions: "Actions",
      },
      noAtRisk: "No at-risk students at this time.",
    },
    scholarships: {
      title: "Scholarship Management",
      applications: {
        title: "Applications",
        search: "Search applicants...",
        review: "Review",
        score: "Score",
        bulk: "Bulk Action",
      },
      triage: {
        title: "Application Triage",
        columns: {
          new: "New",
          underReview: "Under Review",
          committee: "Committee Review",
          readyToAward: "Ready to Award",
        },
      },
      committee: {
        title: "Committee Scoring",
        blind: "Blind Review",
        rubric: "Scoring Rubric",
        submit: "Submit Score",
      },
      decisions: {
        title: "Award Decisions",
        rank: "Rank",
        award: "Award",
        generateOffer: "Generate Offer",
      },
      configure: {
        title: "Configure Scholarship",
        name: "Scholarship Name",
        description: "Description",
        eligibility: "Eligibility Criteria",
        awardAmount: "Award Amount",
        deadline: "Application Deadline",
        requiredDocs: "Required Documents",
        save: "Save Scholarship",
      },
      opportunities: {
        title: "Scholarship Opportunities",
        create: "Create Scholarship",
        edit: "Edit",
        delete: "Delete",
        viewApplicants: "View Applicants",
        statusOpen: "Open",
        statusClosed: "Closed",
        statusDraft: "Draft",
      },
    },
    financialAid: {
      title: "Financial Aid",
      stats: {
        applications: "Applications",
        pendingReviews: "Pending Reviews",
        underReview: "Under Review",
        readyForAward: "Ready for Award",
      },
      quickLinks: {
        applications: "Applications",
        configure: "Configure",
      },
    },
    nsfas: {
      registry: {
        title: "NSFAS Students",
        subtitle: "Students receiving NSFAS financial aid · {count} beneficiaries",
        viewReports: "View tracking reports",
        searchPlaceholder: "Search by name, email or student ID…",
        results: "{count} results",
        loadError: "Failed to load NSFAS students",
        noResults: "No NSFAS beneficiaries found",
        colStudent: "Student",
        colProgramme: "Programme",
        colGender: "Gender",
        colRegion: "Home province",
        colAwardStatus: "Award Status",
        colGpa: "GPA",
        colSupport: "NSFAS Support (KES)",
        colDisbursed: "Disbursed (KES)",
        colFeeBalance: "Fee Balance (KES)",
        colDisability: "Disability",
        colRisk: "Risk",
        notRecorded: "—",
      },
      reports: {
        title: "NSFAS Student Tracking Reports",
        subtitle:
          "Interactive view of NSFAS support disbursed and beneficiary academic performance — filter, sort, and drill down for institutional review or NSFAS reporting",
        visualAnalytics: "Visual analytics",
        viewStudentList: "View student list",
        loadError: "Failed to load NSFAS tracking report",
        noData: "No NSFAS beneficiaries found for this institution.",
        kpiBeneficiaries: "NSFAS Beneficiaries",
        kpiSupport: "Total Support Awarded",
        kpiDisbursed: "Total Disbursed to Date",
        kpiOutstanding: "Outstanding Fee Balance",
        kpiGpa: "Average GPA",
        kpiAttendance: "Average Attendance",
        kpiDisability: "Beneficiaries with a Disability",
        kpiAtRisk: "High Risk / Critical",
        kpiAtRiskSub: "Click to filter · Continuation at risk",
        awardStatus: "Award Status",
        academicStanding: "Academic Standing",
        continuationRisk: "Continuation Risk",
        allowanceTotals: "Allowance Category Totals (KES)",
        disabilityImpact: "Disability Support Impact",
        withDisability: "Beneficiaries with a disability",
        disabilityTopUp: "Disability top-up disbursed",
        avgGpaWith: "Avg GPA — with disability",
        avgGpaWithout: "Avg GPA — without disability",
        studentTable: "Support vs. Academic Performance — Per Student",
        searchPlaceholder: "Search students…",
        filterAll: "All",
        filterAtRisk: "At risk",
        activeFilters: "Active filters:",
        clearFilters: "Clear filters",
        showingCount: "Showing {shown} of {total} students",
        noMatch: "No students match the current filters",
        noBreakdown: "No data available",
        colStudent: "Student",
        colProgramme: "Programme",
        colAwardStatus: "Award Status",
        colSupport: "Support Awarded (KES)",
        colDisbursed: "Disbursed (KES)",
        colOutstanding: "Outstanding (KES)",
        colGpa: "GPA",
        colAttendance: "Attendance",
        colStanding: "Standing",
        colRisk: "Risk",
        filterRegion: "Home province",
        filterGender: "Gender",
        filterDisability: "Disability",
        filterDepartment: "Department",
        filterWithDisability: "With disability",
        filterWithoutDisability: "Without disability",
      },
      analytics: {
        badge: "Visual analytics",
        title: "NSFAS Visual Analytics",
        subtitle:
          "Interactive charts for NSFAS support, disbursements, and academic outcomes — click any chart segment to drill into the detailed report",
        backToReports: "Detailed reports",
        viewStudents: "Student list",
        loadError: "Failed to load NSFAS analytics",
        noData: "No NSFAS beneficiaries found for this institution.",
        kpiBeneficiaries: "Beneficiaries",
        kpiSupport: "Total support awarded",
        kpiDisbursed: "Total disbursed",
        kpiAtRisk: "High risk / critical",
        awardStatusChart: "Award status distribution",
        riskChart: "Continuation risk",
        standingChart: "Academic standing",
        allowanceChart: "Allowance categories",
        allowanceChartSub: "Total annual support by category (KES)",
        disbursementChart: "Disbursement vs outstanding",
        disbursementChartSub: "Share of awarded support disbursed to date",
        disbursed: "Disbursed",
        outstanding: "Outstanding",
        disbursedPct: "disbursed",
        deptChart: "Beneficiaries by department",
        deptChartSub: "Top departments by NSFAS headcount",
        performanceChart: "Support vs GPA",
        performanceChartSub: "Top beneficiaries — annual support (K) vs GPA",
        supportK: "Support (K KES)",
        clickToFilter: "Click to filter detailed report",
        students: "students",
        filterHint: "Opening detailed report filtered by: {label}",
        demographicsTitle: "Beneficiary demographics",
        demographicsSub: "Home province, gender, and disability — decision-ready profile of NSFAS beneficiaries",
        regionChart: "Home province (South Africa)",
        genderChart: "Gender",
        disabilityChart: "Disability status",
        disabilityChartSub: "Beneficiaries with registered disability support needs",
        withDisability: "With disability",
        withoutDisability: "Without disability",
        noRegionData: "No home province data recorded",
        filterPanel: "Explore & filter",
        filterPanelSub: "Combine region, gender, disability, and more — all charts update together",
        filterRegion: "Home province",
        filterGender: "Gender",
        filterDisability: "Disability",
        filterDepartment: "Department",
        filterAny: "Any",
        filterWithDisability: "With disability",
        filterWithoutDisability: "Without disability",
        filteredView: "Filtered view",
        summaryView: "Full summary",
        showingFiltered: "Showing {count} of {total} beneficiaries",
        clickToToggle: "Click a chart segment to add or remove a filter",
        noMatchFilter: "No beneficiaries match the current filters",
        viewDetailedReport: "Open detailed report",
        drillDownTable: "Matching beneficiaries",
        viewAllMatching: "View all {count} in detailed report",
        screenOptions: "Screen options",
        screenOptionsSub: "Show, hide, and reorder dashboard widgets",
        resetLayout: "Reset layout",
        dragToReorder: "Drag rows or use arrows to reorder widgets",
        moveUp: "Move up",
        moveDown: "Move down",
        alwaysVisible: "always visible",
        widgetFilters: "Filter panel",
        widgetKpis: "Key metrics",
        widgetRegion: "Home province chart",
        widgetGender: "Gender chart",
        widgetDisability: "Disability chart",
        widgetAward: "Award status chart",
        widgetRisk: "Continuation risk chart",
        widgetStanding: "Academic standing chart",
        widgetAllowance: "Allowance categories",
        widgetDisbursement: "Disbursement vs outstanding",
        widgetDept: "Department chart",
        widgetPerformance: "Support vs GPA",
        widgetDrillTable: "Matching beneficiaries table",
        filterMultiHint: "Select multiple values to build custom reports",
        filterSelected: "selected",
      },
      executive: {
        title: "NSFAS financial aid",
        subtitle: "Government-funded student support snapshot for leadership review",
        beneficiaries: "Beneficiaries",
        disbursed: "Disbursed to date",
        atRisk: "At continuation risk",
        viewAnalytics: "Open NSFAS analytics",
        viewReports: "Detailed tracking report",
        unavailable: "No NSFAS beneficiaries on record for this institution.",
      },
    },
    studentDetail: {
      back: "Students",
      loadError: "Failed to load student record",
      notFound: "Student not found.",
      contact: "Contact & programme",
      demographics: "Demographics",
      performance: "Academic performance",
      email: "Email",
      phone: "Phone",
      programme: "Programme",
      gender: "Gender",
      homeProvince: "Home province (South Africa)",
      disability: "Disability",
      nationality: "Nationality",
      gpa: "GPA",
      attendance: "Attendance",
      yes: "Yes",
      no: "No",
      notRecorded: "Not recorded",
      coursesPassed: "Courses passed",
      coursesFailed: "Courses failed",
      inProgress: "In progress",
      passed: "Passed",
      failed: "Failed",
      enrolled: "In progress",
      courseOutcomes: "Course outcomes",
      scoresByCourse: "Scores by course",
      scoresSub: "Midterm / final scores for enrolled modules",
      score: "Score",
      noCourses: "No course enrollments on record",
      noScores: "No scored courses yet",
      creditProgress: "Credit progress",
      credits: "credits",
      tabCourses: "Courses & performance",
      tabFinancial: "Financial",
      tabScholarships: "Scholarships",
      colCode: "Code",
      colTitle: "Course",
      colStatus: "Status",
      colOutcome: "Outcome",
      colMidterm: "Midterm",
      colFinal: "Final",
      colGrade: "Grade",
      colCredits: "Credits",
      totalFees: "Total fees",
      totalPaid: "Total paid",
      balanceDue: "Balance due",
      paymentId: "Payment ID",
      date: "Date",
      amount: "Amount",
      method: "Method",
      status: "Status",
      noPayments: "No payment records",
      scholarshipName: "Scholarship",
      appliedDate: "Applied",
      scholarshipStatus: "Status",
      noScholarships: "No scholarship applications",
      department: "School / faculty",
      financialSupport: "Financial support",
      noFinancialSupport: "No NSFAS or scholarship support on record",
      nsfasSupport: "NSFAS financial aid",
      nsfasBeneficiary: "NSFAS beneficiary",
      nsfasAwarded: "Awarded",
      nsfasDisbursed: "Disbursed",
      scholarshipSupport: "Scholarships & awards",
      filterAcademicYear: "Academic year",
      filterSemester: "Semester",
      filterAll: "All",
      filterAllPeriods: "All periods",
      noCoursesFilter: "No courses match the selected period",
      noScoresFilter: "No scores for the selected period",
      colPeriod: "Period",
    },
    grants: {
      title: "Grants Management",
      applications: { title: "Grant Applications" },
      opportunities: { title: "Grant Opportunities" },
      configure: { title: "Configure Grant" },
      lifecycle: { title: "Grant Lifecycle" },
    },
    analytics: {
      title: "Analytics",
      executiveTitle: "Institutional Analytics",
      executiveSubtitle: "Decision-ready snapshot for leadership — interactive views, no exhaustive lists",
      executiveBadge: "Executive briefing",
      enrollment: "Enrollment Analytics",
      performance: "Student Performance",
      programs: "Program Analysis",
      exportReport: "Export Report",
      retentionTab: "Retention",
      screenOptions: "Screen options",
      screenOptionsSub: "Show, hide, and reorder widgets on this tab",
      resetLayout: "Reset layout",
      dragToReorder: "Drag rows or use arrows to reorder widgets",
      moveUp: "Move up",
      moveDown: "Move down",
      alwaysVisible: "always visible",
      retention: {
        oneYear: "1-year retention",
        fourYear: "4-year graduation",
        cohortsTracked: "{count} cohorts tracked",
        snapshot: "Snapshot {date}",
        cohortAverage: "Cohort average",
        initialSize: "Initial cohort size",
        acrossSnapshots: "Across latest cohort snapshots",
        graduated: "Graduated",
        fromTracked: "From tracked cohorts",
        byCohort: "Retention by cohort",
        byCohortSub: "Latest snapshot per intake cohort",
        retentionSeries: "1-yr retention %",
        graduationSeries: "4-yr graduation %",
        unavailable: "Cohort retention metrics are not available yet. Seed or sync retention snapshots to unlock this view.",
      },
    },
    rankings: {
      title: "Rankings",
      gpaRankings: "GPA Rankings",
      coursePerformance: "Course Performance",
      scholarshipRates: "Scholarship Success Rates",
    },
    universityRankings: {
      exportPdf: "Export PDF",
      exportPdfTooltip: "Choose a ranking and download it as its own PDF report",
      exportDialogTitle: "Export ranking report",
      exportDialogSubtitle:
        "Select one ranking system. Each download is a separate report with institutional overview and that ranking's readiness detail.",
      downloadReport: "Download",
      reportDocumentTitle: "{institution} - {ranking} report",
      printReportHeading: "{ranking} readiness report",
    },
    support: {
      title: "Student Support Management",
      interventionLog: "Intervention Log",
      ticketQueue: "Support Ticket Queue",
      journey: "Student Journey",
    },
  },

  // ─── Global Admin ──────────────────────────────────────────────────────────
  globalAdmin: {
    portal: "Global Admin",
    loading: "Loading admin portal...",
    login: {
      subtitle: "Global admin login",
      signInBtn: "Sign In",
      backToHome: "Back to home",
    },
    nav: {
      dashboard: "Dashboard",
      institutions: "Institutions",
      users: "Users",
      analytics: "Analytics",
      settings: "Settings",
      activity: "Activity Log",
    },
    dashboard: {
      title: "Global Admin Dashboard",
      stats: {
        totalInstitutions: "Total Institutions",
        totalUsers: "Total Users",
        activeUsers: "Active Users",
        systemHealth: "System Health",
      },
    },
    institutions: {
      title: "Institution Management",
      pageTitle: "Institutions",
      subtitle: "Manage all institutions",
      createBtn: "Add Institution",
      searchPlaceholder: "Search institutions...",
      allStatus: "All statuses",
      empty: "No institutions yet. Click 'Add Institution' to get started.",
      noMatch: "No institutions match your filters.",
      fetchError: "Failed to fetch institutions",
      createError: "Failed to create institution",
      updateError: "Failed to update institution",
      createAdminError: "Failed to create admin",
      addDomainError: "Failed to add domain",
      deactivateError: "Failed to deactivate institution",
      activateError: "Failed to activate institution",
      deleteError: "Failed to delete institution",
      table: {
        name: "Institution",
        slug: "Slug",
        domains: "Domains",
        contact: "Contact",
        status: "Status",
        activeUsers: "Active Users",
        storage: "Storage",
        actions: "Actions",
      },
      manage: "Manage",
      actionMenu: {
        addDomain: "Add domain",
        addAdmin: "Add admin",
        more: "More actions",
        deactivate: "Deactivate",
        activate: "Activate",
      },
      confirmDeleteTitle: "Delete institution?",
      confirmDeactivateTitle: "Deactivate institution?",
      confirmDelete: "Are you sure you want to delete \"{name}\"? This cannot be undone and will fail if the institution has users.",
      confirmDeactivate: "Are you sure you want to deactivate \"{name}\"? Users will not be able to access this institution.",
      editTitle: "Edit institution",
      createTitle: "Create new institution",
      fields: {
        name: "Institution name",
        slug: "Slug",
        slugHelper: "URL-friendly identifier (e.g., harvard-university)",
        slugCreateLabel: "Slug (URL identifier)",
        slugCreateHelper: "Auto-generated from name, or customize manually",
        contactEmail: "Contact email",
        domain: "Domain",
        domainPlaceholder: "university.edu",
        domainHelper: "Enter the domain without the @ sign",
        fullName: "Full name",
        password: "Password",
      },
      createAdminTitle: "Create admin for {name}",
      addDomainTitle: "Add domain to {name}",
      availableDomains: "Available domains: {domains}",
      noDomains: "No domains configured for this institution",
      createAdminBtn: "Create admin",
      addDomainBtn: "Add domain",
      modules: {
        title: "Accessible modules",
        subtitle: "Choose which modules students and other accounts at this institution can use.",
        colModule: "Module",
        colStudent: "Students",
        colStaff: "Staff and other accounts",
        staffOnly: "Staff only",
        saving: "Saving…",
        saveError: "Failed to update modules",
        enrollment: "Enrollment and student success",
        scholarships: "Scholarship and financial aid",
        support: "Student support",
        grants: "Grants and research",
        rankings: "University rankings",
      },
    },
    users: {
      title: "User Management",
      createBtn: "Create User",
      table: {
        name: "Name",
        email: "Email",
        role: "Role",
        institution: "Institution",
        status: "Status",
        actions: "Actions",
      },
    },
    analytics: {
      title: "System Analytics",
      institutions: "Institutions",
      userGrowth: "User Growth",
      featureUsage: "Feature Usage",
      performance: "Performance",
    },
    settings: {
      title: "System Settings",
      emailTemplates: "Email Templates",
      parameters: "System Parameters",
      featureFlags: "Feature Flags",
      maintenance: "Maintenance Mode",
      save: "Save Settings",
    },
    activity: {
      title: "Activity Log",
      search: "Search logs...",
      filter: "Filter",
      export: "Export",
      table: {
        timestamp: "Timestamp",
        user: "User",
        action: "Action",
        details: "Details",
      },
    },
  },

  // ─── Institution Admin ─────────────────────────────────────────────────────
  institutionAdmin: {
    portal: "Institution Admin",
    loading: "Loading portal...",
    login: {
      subtitle: "Institution admin login",
      signInBtn: "Sign In",
      backToHome: "Back to home",
    },
    nav: {
      dashboard: "Dashboard",
      users: "Users",
      access: "Role access",
      profile: "Institution Profile",
      domains: "Email Domains",
      analytics: "Analytics",
      grants: "Grant Portfolio",
      activity: "Activity Log",
    },
    navSections: {
      overview: "Overview",
      management: "Management",
      system: "System",
    },
    dashboard: {
      title: "Institution Dashboard",
      stats: {
        students: "Students",
        programs: "Programs",
        enrollments: "Enrollments",
        users: "Users",
      },
      quickLinks: "Quick Links",
    },
    access: {
      title: "Role access",
      subtitle:
        "Choose whole modules or specific sidebar items each role can open. Dashboard, profile, and analytics stay available.",
      institutionCeiling: "Modules enabled for this institution",
      institutionCeilingHint:
        "These are set by the global administrator. You can only grant roles access to modules that are already on.",
      roleHeading: "Access by role",
      roleHint:
        "Select a role, then tick a whole module or expand and pick individual sidebar items. Changes save immediately.",
      modulesForRole: "Access for {role}",
      moduleEnabledHint: "Appears in this role's staff sidebar when enabled",
      moduleDisabledByInstitution: "Not enabled for this institution - ask a global admin to turn it on first",
      wholeModuleOn: "Entire module enabled - all sidebar items below are included",
      partialModuleOn: "Partial access - only the selected sidebar items below",
      moduleOff: "Module off for this role - tick the module or individual items to grant access",
      itemsHint: "Or select specific sidebar items:",
      saving: "Saving…",
      saved: "Role access updated",
      fetchError: "Could not load role access settings.",
      saveError: "Could not save role access settings.",
      navItems: {
        atRisk: "At-Risk Students",
        students: "Students",
        support: "Student Support",
        scholarships: "Scholarships",
        financialAid: "Financial aid dashboard",
        applications: "Scholarship applications",
        triage: "Triage & verification",
        decisions: "Review outcome and awards",
        scholarshipOpportunities: "Scholarship opportunities",
        configureScholarships: "Configure scholarships",
        grants: "Grants & research",
        grantLifecycle: "Grant lifecycle pipeline",
        grantApplications: "Grant applications",
        grantOpportunities: "Grant opportunities",
        configureGrants: "Configure grants",
        rankings: "University rankings",
        rankingsExecutive: "University Rankings Summary",
      },
    },
    users: {
      title: "Users & Roles",
      subtitle: "Manage institution users and their access roles · {count} total",
      createBtn: "Add User",
      searchPlaceholder: "Search users...",
      allRoles: "All Roles",
      userColumn: "User",
      moreActions: "More",
      fetchError: "Failed to fetch data",
      createError: "Failed to create user",
      updateError: "Failed to update user",
      activateError: "Failed to activate user",
      deactivateError: "Failed to deactivate user",
      deleteError: "Failed to delete user",
      empty: "No users yet. Click Add User to get started.",
      noMatch: "No users match your filters.",
      editUser: "Edit User",
      deactivate: "Deactivate",
      activate: "Activate",
      createTitle: "Add New User",
      fullName: "Full Name",
      emailMustMatch: "Must match: {domains}",
      password: "Password",
      createUserBtn: "Create User",
      editTitle: "Edit User",
      deleteTitle: "Delete User?",
      deleteConfirm: "Are you sure you want to delete {name}? This action cannot be undone.",
      changePassword: "Change Password",
      changePasswordTitle: "Change Password",
      changePasswordFor: "Set a new password for {name}.",
      newPassword: "New Password",
      confirmPassword: "Confirm Password",
      passwordHelper: "At least 8 characters",
      passwordTooShort: "Password must be at least 8 characters",
      passwordsNoMatch: "Passwords do not match",
      passwordUpdated: "Password updated",
      passwordError: "Failed to update password",
      savePassword: "Update Password",
      roles: {
        vice_chancellor: "Vice Chancellor",
        registrar: "Registrar",
        scholarship_office: "Financial Aid Officer",
        student: "Student",
        student_services: "Student Services",
        research_office: "Research Office",
        institution_admin: "Institution Admin",
      },
      table: {
        name: "Name",
        email: "Email",
        role: "Role",
        status: "Status",
        actions: "Actions",
      },
    },
    profile: {
      title: "Institution Profile",
      subtitle: "View and manage your institution information",
      name: "Institution Name",
      slug: "Slug",
      domain: "Domain",
      contactInfo: "Contact Information",
      contactEmail: "Contact Email",
      address: "Address",
      createdOn: "Created On",
      logoUpload: "Upload Logo",
      logoChange: "Change logo",
      logoRemove: "Remove logo",
      logoHint: "PNG, JPEG, or WebP. Maximum 2 MB.",
      logoUpdated: "Logo updated",
      logoRemoved: "Logo removed",
      academicCalendar: "Academic Calendar",
      save: "Save Changes",
      editProfile: "Edit Profile",
      editTitle: "Edit Institution Profile",
      updateProfile: "Update Profile",
      contactEmailHelper: "Primary contact email for the institution",
      addressHelper: "Physical address of the institution",
      authorizedDomains: "Authorized Email Domains",
      authorizedDomainsHint: "Users with these domains can sign up or be added to your institution.",
      noDomains: "No domains configured.",
      notSet: "Not set",
      fetchError: "Could not load the institution profile.",
      updateError: "Could not update the profile.",
      logoError: "Could not upload the logo.",
      logoTypeError: "Use a PNG, JPEG, or WebP image.",
      logoSizeError: "The logo must be 2 MB or smaller.",
      fallbackName: "Institution",
    },
    domains: {
      title: "Email Domains",
      addDomain: "Add Domain",
      verify: "Verify",
      verified: "Verified",
      unverified: "Unverified",
      table: {
        domain: "Domain",
        status: "Status",
        addedOn: "Added On",
        actions: "Actions",
      },
    },
    analytics: {
      title: "Institution Analytics",
      departments: "Department Breakdown",
      programs: "Program Analysis",
      cohortAnalysis: "Cohort Analysis",
      trends: "Trend Reports",
    },
    grants: {
      title: "Grant Portfolio",
      budget: "Budget Allocation",
      fundSources: "Fund Sources",
      disbursement: "Disbursement Tracking",
    },
    activity: {
      title: "Activity Log",
      search: "Search logs...",
      table: {
        timestamp: "Timestamp",
        user: "User",
        action: "Action",
        details: "Details",
      },
    },
  },

  // ─── Reviewer ──────────────────────────────────────────────────────────────
  reviewer: {
    portal: "Reviewer Portal",
    loading: "Loading reviewer portal...",
    nav: {
      dashboard: "Dashboard",
      tasks: "Review Tasks",
      history: "Review History",
    },
    dashboard: {
      title: "Reviewer Dashboard",
      stats: {
        pending: "Pending Tasks",
        completed: "Completed Reviews",
        totalAssigned: "Total Assigned",
        avgScore: "Average Score",
      },
      openQueueCta: "Open Review Queue",
      noTasksMsg: "No pending review tasks.",
    },
    tasks: {
      title: "Review Queue",
      blindApplication: "Application (Blind)",
      scoringRubric: "Scoring Rubric",
      submitScore: "Submit Score",
      submitting: "Submitting...",
      noTasks: "Your review queue is empty.",
    },
    history: {
      title: "Review History",
      table: {
        application: "Application",
        score: "Score",
        submittedOn: "Submitted On",
      },
    },
  },

  // ─── Reviewer invite & recommendation portal ───────────────────────────────
  reviewerInvite: {
    title: "Reviewer Invitation",
    accept: "Accept Invitation",
    decline: "Decline",
    description: "You have been invited to review scholarship applications.",
  },
  recommendationPortal: {
    title: "Recommendation Letter",
    verify: "Verify & Submit",
    description: "Please verify your recommendation letter below.",
  },

  // ─── Book a demo ──────────────────────────────────────────────────────────
  bookDemo: {
    title: "Book a demo",
    subtitle:
      "Tell us about your institution and we will schedule a brief demonstration of TemplumIS.",
    fields: {
      fullName: "Full name",
      email: "Work email",
      institution: "Institution",
      jobTitle: "Role / job title",
      phone: "Phone",
      message: "How can we help?",
    },
    placeholders: {
      message: "Share your ranking, reporting, or student-success priorities.",
    },
    submitBtn: "Request a demo",
    submitting: "Sending request...",
    success: "Thank you. Your demo request has been sent. We will contact you shortly.",
    sendError: "We could not send your request. Please try again or email info@templumis.org.",
    requiredFields: "Please complete all required fields.",
    invalidEmail: "Enter a valid work email address.",
    tooManyRequests: "You recently sent a request. Please wait a moment and try again.",
  },

  // ─── FAQs ──────────────────────────────────────────────────────────────────
  faqs: {
    title: "Frequently Asked Questions",
    subtitle:
      "Connected intelligence for universities - how TemplumIS turns fragmented institutional data into strategic insight for planning, performance, accreditation, and rankings.",
    categories: {
      about: {
        title: "About TemplumIS",
        intro: "What the platform is and why institutions adopt it.",
        items: {
          whatIs: {
            q: "What is TemplumIS?",
            a: "TemplumIS is a university intelligence platform designed by TCC Africa to help higher education institutions transform fragmented institutional data into actionable intelligence for strategic decision-making, performance management, accreditation, and university rankings.\n\nIt provides a centralized intelligence layer that integrates data from across the university ecosystem, so leadership can access reliable insights through a single platform.",
          },
          problem: {
            q: "What challenge does TemplumIS address?",
            a: "Universities are increasingly required to provide accurate, timely data for institutional planning, quality assurance, government reporting, internationalization, research performance, and global ranking frameworks.\n\nIn practice, much of that information remains dispersed across offices, departments, and standalone systems. Collection and reporting become time-consuming and resource-intensive. TemplumIS consolidates this landscape so institutions can work from one coherent source of truth.",
          },
          connectedIntelligence: {
            q: "What does “Connected Intelligence for Universities” mean?",
            a: "It means TemplumIS does not replace every operational system overnight. Instead, it connects academic, research, internationalization, finance, quality assurance, and student information sources into a unified environment - turning operational records into leadership-ready intelligence.",
          },
        },
      },
      capabilities: {
        title: "Platform capabilities",
        intro: "How TemplumIS supports day-to-day operations and strategic oversight.",
        items: {
          consolidate: {
            q: "What kinds of institutional data can TemplumIS bring together?",
            a: "TemplumIS consolidates data from across the university ecosystem - including academic records, research activity, internationalization, finance, quality assurance, and student information systems - into a unified environment for analysis and reporting.",
          },
          reporting: {
            q: "How does TemplumIS reduce the burden of institutional reporting?",
            a: "By centralizing indicators and automating collection where possible, TemplumIS reduces the administrative effort of assembling reports for planning, regulatory submissions, quality assurance, and ranking exercises. Teams spend less time chasing spreadsheets and more time validating and acting on insights.",
          },
          leadership: {
            q: "What do university leaders see in the platform?",
            a: "Leaders access real-time dashboards and performance indicators that support evidence-based decision-making. The platform strengthens institutional planning with clearer visibility into performance trends and, where configured, predictive analytics to anticipate risk and opportunity.",
          },
          studentResearch: {
            q: "How does TemplumIS support student success and research performance?",
            a: "On the student side, institutions can track enrollment, retention, progression, graduation, employability signals, and academic performance.\n\nOn the research side, TemplumIS helps monitor outputs, grants, collaborations, citations, patents, and innovation activities - linking research investment to visible institutional outcomes.",
          },
          accreditation: {
            q: "Does TemplumIS support accreditation and quality assurance?",
            a: "Yes. The platform is designed to support accreditation, quality assurance, and regulatory reporting by keeping the underlying evidence organized, current, and easier to retrieve when review cycles arrive.",
          },
        },
      },
      rankings: {
        title: "University rankings",
        intro: "Ranking readiness and continuous performance monitoring.",
        items: {
          readiness: {
            q: "How does TemplumIS support university ranking readiness?",
            a: "A core strength of TemplumIS is ranking readiness and performance monitoring. Institutions can systematically collect, organize, validate, and manage the data required for major ranking frameworks - instead of assembling submissions from scratch each year across multiple offices.",
          },
          frameworks: {
            q: "Which ranking frameworks can institutions prepare for?",
            a: "TemplumIS supports structured indicator management for major global and regional frameworks, including:\n\n• QS World University Rankings\n• Times Higher Education (THE) Rankings\n• Webometrics Rankings\n• Academic Ranking of World Universities (ARWU / Shanghai)\n• Regional and national ranking frameworks\n\nThis helps institutions improve consistency and preparedness for annual ranking cycles.",
          },
          repository: {
            q: "How is ranking data managed differently with TemplumIS?",
            a: "Rather than relying on multiple offices to manually compile ranking submissions, TemplumIS provides a structured institutional repository where ranking-related indicators are continuously captured, updated, and verified.\n\nThat approach reduces reporting effort while improving data quality, consistency, and institutional preparedness - strengthening global visibility, competitiveness, and reputation over time.",
          },
        },
      },
      gettingStarted: {
        title: "Getting started",
        intro: "Access, roles, demonstrations, and support.",
        items: {
          whoUses: {
            q: "Who uses TemplumIS?",
            a: "TemplumIS serves the full institutional community through role-based portals: students, registrars and student services, scholarship and financial-aid offices, research offices, reviewers, advisors and sponsors, and institution or global administrators.",
          },
          accounts: {
            q: "How do users get access?",
            a: "Students and staff typically sign up or are invited once their institution is provisioned on the platform. Institution and global administrators use dedicated login pages. Access is always scoped to the user’s role and the modules enabled for that institution.",
          },
          demo: {
            q: "Can we request a demonstration?",
            aBefore: "Yes. Institutions that want to explore how TemplumIS supports strategic priorities and ranking ambitions can ",
            linkLabel: "request a brief meeting or demonstration",
            aAfter: ". The TemplumIS team will get in touch to schedule a conversation.",
          },
          help: {
            q: "Where can I find documentation or contact support?",
            aBefore: "Product guides and technical references are available in ",
            aAfter:
              ". For institutional enquiries, contact Training Centre in Communication (TCC Africa), Gecaga Institute Building, Nairobi, Kenya.",
          },
        },
      },
    },
  },
};

export default en;
